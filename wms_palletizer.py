...
def _load_config() -> configparser.ConfigParser:
    ...
    if os.path.isfile(_CONFIG_FILE):
        ...
        # Убрали секцию [security] с логином из файла
        cfg["security"] = {
...
# ══════════════════════════════════════════════════════════════
#  НАСТРОЙКИ ПРИЛОЖЕНИЯ И БЕЗОПАСНОСТЬ
# ══════════════════════════════════════════════════════════════
...
# 🛡️ ЖЕСТКИЙ СПИСОК РАЗРЕШЕННЫХ ПОЛЬЗОВАТЕЛЕЙ (недоступен из config.ini)
ALLOWED_USERS_LIST = ["u001164", "u000644"] 

# Из конфига берем только некритичные параметры
EXPIRATION_DATE = _cfg.get("security", "expiration_date", fallback="2026-04-30")
...
# ══════════════════════════════════════════════════════════════
#  GUI — ОКНО АВТОРИЗАЦИИ (С ЗАЩИТОЙ)
# ══════════════════════════════════════════════════════════════
...
class LoginWindow(ctk.CTk):
    def __init__(self):
        ...
        # 🛡️ Запуск проверки безопасности при открытии
    ...
    def _run_security_checks(self):
        """Проверка даты истечения (Оффлайн защита)"""
        try:
            expiry_date = datetime.strptime(EXPIRATION_DATE, "%Y-%m-%d")
            if datetime.now() > expiry_date:
                self._lock_app("⏳ Срок действия этой версии истек.\nОбратитесь к разработчику.")
                return
        except Exception:
            pass
        

    def _lock_app(self, message: str):
        """Блокирует интерфейс ввода"""
        self.login_btn.configure(state="disabled", fg_color=C_TEXT_SEC)
        self.login_entry.configure(state="disabled")
        self.pass_entry.configure(state="disabled")
        self.status_label.configure(text=message, text_color=C_DANGER)
